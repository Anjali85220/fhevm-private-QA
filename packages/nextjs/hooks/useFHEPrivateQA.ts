...
/**
 * @hook useFHEPrivateQA
 * @notice Manages encryption, decryption, and interaction with the FHEPrivateQA contract.
 */
export const useFHEPrivateQA = ({
  instance,
  initialMockChains,
}: {
  ...
  const { data: fhePrivateQA } = useDeployedContractInfo({
    contractName: "FHEPrivateQA",
    chainId: allowedChainId,
  });

  type FHEPrivateQAInfo = Contract<"FHEPrivateQA"> & { chainId?: number };

  const [message, setMessage] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);

  const hasContract = Boolean(fhePrivateQA?.address && fhePrivateQA?.abi);
  ...
  const getContract = (mode: "read" | "write") => {
    ...
    return new ethers.Contract(fhePrivateQA!.address, (fhePrivateQA as FHEPrivateQAInfo).abi, providerOrSigner);
  };

  // Fetch encrypted answer
  const { data: encryptedAnswer, refetch: refreshAnswer } = useReadContract({
    address: hasContract ? (fhePrivateQA!.address as `0x${string}`) : undefined,
    abi: hasContract ? ((fhePrivateQA as FHEPrivateQAInfo).abi as any) : undefined,
  ...
  const requests = useMemo(() => {
    ...
    return [
      {
        handle: answerHandle,
        contractAddress: fhePrivateQA!.address,
      },
    ] as const;
  }, [hasContract, fhePrivateQA?.address, answerHandle]);
  ...
  const getEncryptionMethodFor = (functionName: "submitAnswer") => {
    const functionAbi = fhePrivateQA?.abi.find(item => item.type === "function" && item.name === functionName);
    if (!functionAbi) {
      return {
  ...
  const submitAnswer = useCallback(
    async (answerString: string) => {
      ...
      try {
        ...
        const params = buildParamsFromAbi(enc, [...fhePrivateQA!.abi] as any[], "submitAnswer");

        const tx = await writeContract.submitAnswer(...params, {
          gasLimit: 400_000,
        });
        await tx.wait();
        ...
        setMessage("❌ Failed to submit the answer.");
      } finally {
        setIsProcessing(false);
      }
    },
    [encryptWith, getContract, fhePrivateQA?.abi, isProcessing],
  );
