...
import { FHEPrivateQA, FHEPrivateQA__factory } from "../types";
...
async function deployFixture() {
  const factory = (await ethers.getContractFactory("FHEPrivateQA")) as FHEPrivateQA__factory;
  const quizContract = (await factory.deploy()) as FHEPrivateQA;
  const quizContractAddress = await quizContract.getAddress();
  return { quizContract, quizContractAddress };
}

describe("FHEPrivateQA", function () {
  let signers: Signers;
  let quizContract: FHEPrivateQA;
